...
<div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                <div>
                    ...
                    <input x-model="siteurl" name="siteurl" type="text" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring">
                </div>
                <div>
...
fetch('<?php echo siteurl; ?>/admin/api.php?screenName='+screenName+'&mail='+mail,{method: 'POST',}).then(data => data.json()).then(data => {if(data.status=='-1'){Notifications(data.info,'error');}else{Notifications('设置保存完成！');
}});
...
<!--登录设置-->
<div class="bg-white dark:bg-black p-2 sm:p-6 mt-9" x-data="{username:'<?php echo $config['username']; ?>',password:'',confirm:'',}"> 
<h2 class="flex items-center text-xl font-medium text-gray-800 capitalize dark:text-white md:text-2xl"><svg class="w-6 h-6 inline text-blue-700 mr-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>网站设置</h2>
<div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                <div>
