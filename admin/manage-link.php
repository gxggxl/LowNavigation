...
<?php if(!empty($res)): ?>
<div class="bg-white dark:bg-black p-2 sm:p-6 mt-9" x-data="{name:'<?php echo $res['name']; ?>',dis:'<?php echo $res['dis']; ?>',site:'<?php echo $res['site']; ?>',mid:'<?php echo $res['mid']; ?>',}">
...
<input x-model="name" type='text' name="name" placeholder="请输入网站名字" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring" />
</div>
<div>
...
fetch('<?php echo siteurl; ?>/admin/api.php?name='+name+'&dis='+dis+'&site='+site+'&mid='+mid<?php if(!empty($res)){echo "+'&edit=".$res['name']."'";} ?>,{method: 'POST',}).then(data => data.json()).then(data => {if(data.status=='-1'){Notifications(data.info,'error');}else{a=data;name='';dis='';site='';Notifications('链接修改完成！');
}});
...
@dragend.prevent="b=JSON.stringify(a);fetch('<?php echo siteurl; ?>/admin/api.php?link='+b,{method: 'POST',}).then(data => data.json()).then(data => {if(data.status=='-1'){Notifications(data.info,'error');}});"
...
<div class="p-2 w-full flex justify-between h-full bg-white dark:bg-gray-900 items-center border-gray-100 dark:border-gray-600 border p-4 rounded-lg hover:shadow-md duration-300 relative cursor-move" draggable="true" :class="{'border-blue-600': dragging === index}" @dragstart="dragging = index" @dragend="dragging = null">
